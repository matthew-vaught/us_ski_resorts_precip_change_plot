...
const proj = d3.geoAlbersUsa().translate([W/2, H/2]).scale(1200);
const path = d3.geoPath(proj);

// Canvas for the heat layer
const canvas = d3.select("#map").append("canvas")
  .attr("width", W).attr("height", H)
  .style("position","relative").style("top", `-${H}px`);
const ctx = canvas.node().getContext("2d");

// Tooltip
const tooltip = d3.select("body").append("div").attr("class","tooltip").style("opacity",0);

// Diverging colors around 0% (brown=drier, green=wetter) — works nicely for precip
const color = d3.scaleDiverging(d3.interpolateBrBG).domain([50, 0, -50]).clamp(true);
...
Promise.all([
  ...
  d3.csv("data/resorts.csv", d3.autoType) // name,state,lat,lon
  ...
  // Index grid by year for fast redraws
  const byYear = d3.group(grid, d => d.year);

  // Resort markers (SVG so they sit above the canvas and are hoverable)
  g.selectAll(".resort").data(resorts).join("circle")
    ...
    .on("mouseover", (event, d) => {
      ...
      // nearest-grid value for the current year
      ...
      if (p && arr.length) {
        ...
        for (const row of arr) {
          ...
          if (!q) continue;
          const dd = (q[0]-p[0])*(q[0]-p[0]) + (q[1]-p[1])*(q[1]-p[1]);
  ...
  // Heatmap drawing for a single year
  function draw(year) {
    ctx.clearRect(0, 0, W, H);
    const arr = byYear.get(year) || [];
    const r = 2.0; // “pixel” size per gridpoint
    for (const d of arr) {
      const p = proj([+d.lon, +d.lat]);
      if (!p) continue;
...
function drawLegend(){
  ...
  const stops = d3.range(0,1.001,0.05).map(t => ({t, c: d3.interpolateBrBG(t)}));
